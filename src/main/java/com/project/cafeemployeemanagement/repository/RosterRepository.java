...
public interface RosterRepository extends JpaRepository<Roster, Long> {
    ...
    Roster findByDatesAndShopOwnerId(@Param("fromDate") Date fromDate, @Param("toDate") Date toDate, @Param("shopOwnerId") Long shopOwnerId);

    @Query("SELECT r FROM Roster AS r WHERE r.toDate > :currentDate and r.employee.id = :shopOwnerId")
    List<Roster> findByToDateAfterAndAndEmployee(@Param("currentDate") Date currentDate , @Param("shopOwnerId") Long shopOwnerId);
}
